...
- **SATCOM (מצב רביעי, ACARS דרך לוויין Inmarsat):** `_satcom_listener` (thread, מאזין
  ...
  **אבל מיקום ADS-C נאמן *רק* כש-`decode_failed=False`, לא סתם "יש `libacars`"
  ...
  רק לצביעה). **המשמעות: אין לנו יותר "הוכחה" ש-ADS-C עובד עד שרואים מיקום
  שמאומת חיצונית** — לוקח ברצינות שהמסקנה הקודמת התבררה כשגויה בפועל. `_libacars_decode`
  ...
  ונכשלנו" — **וגם כדי לחסום מיקום מפוקפק** (ר' למעלה). `_normalize_acars`/
  `_normalize_vdl2` שני המקומות שקוראים ל-`_scan_latlon` מגודרים כעת ב-
  `not decode_failed`. **CPDLC (label AA) נצפה בשפע בקליטה** (14 הודעות ב-33
  דק') אבל ברוב המקרים `decoded` נשאר הודעת-כישלון — לא כי הקוד לא מטפל בזה,
  אלא כי המפענח עצמו נכשל ביישום המקונן. **פענוח CPDLC-לטקסט-קריא עצמו
  (clearance/instruction) עדיין לא מומש** — דורש דגימת JSON גולמית עם
  `cpdlc.err=false` (עדיין לא נצפתה בקליטת שדה) לפני שבונים אקסטרקטור בלי
  לנחש שמות שדות. `src/dst.type` "Aircraft/Ground Earth Station" קובע
