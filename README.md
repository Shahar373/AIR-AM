...
| latency גבוה (עיכוב בשמיעה) | שני גורמים: (1) Icecast `burst-size=0` — `install.sh` מבטיח זאת (גם אם התג חסר בברירת המחדל). (2) rtl_airband מקודד ב-16kbps קבוע-בקוד => הדפדפן ממלא buffer לאט; ההתקנה בונה מחדש ב-**48kbps** כך שהנגן מתחיל מהר ו-latency צונח ל-~3–6 שניות. **חובה להריץ שוב `sudo ./install.sh`** כדי ששני התיקונים יחולו. רצפה מעשית בנגן דפדפן רגיל היא ~3 שניות. |
